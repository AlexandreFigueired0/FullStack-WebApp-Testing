...
public class TestIndex {
	...
	@Test
	public void updateAndRevertCustomerContactsTest() throws IOException {
		final String VAT = "197672337"; // vat of exisiting customer
		final String OLD_PHONE = "914276732";
		final String NEW_PHONE = "123456789"; // new phone number

		// get a specific link
		HtmlAnchor updateCustomerContactsLink = page.getAnchorByHref("updateCustomerContacts.html");
		// click on it
		HtmlPage nextPage = (HtmlPage) updateCustomerContactsLink.openLinkInNewWindow();
		...
		vatInput.setValueAttribute(VAT);
		HtmlInput phoneInput = addCustomerForm.getInputByName("phone");
		phoneInput.setValueAttribute(NEW_PHONE);
		// submit form
		HtmlInput submit = addCustomerForm.getInputByValue("Submit Update");

		// check if report page includes the proper values
		HtmlPage reportPage = submit.click();
		String textReportPage = reportPage.asText();
		assertTrue(textReportPage.contains(NEW_PHONE));
		assertFalse(textReportPage.contains(OLD_PHONE));

		////////////////////// REVERT //////////////////////////////
		vatInput.setValueAttribute(VAT);
		phoneInput.setValueAttribute(OLD_PHONE);

		// check if report page includes the proper values
		HtmlPage reportPage2 = submit.click();
		String textReportPage2 = reportPage2.asText();
		assertFalse(textReportPage2.contains(NEW_PHONE));
		assertTrue(textReportPage2.contains(OLD_PHONE));
	}

	@Test
	public void createTwoSalesAndCloseOneTest() throws IOException, ClassNotFoundException, SQLException {
		final String VAT = "197672337"; // vat of exisiting customer
...
//
		...
		saleIdInput.setValueAttribute("1");
		submit = closeSaleForm.getInputByValue("Close Sale");
		reportPage = submit.click();
		
		assertTrue(textReportPage.contains("O"));
		assertTrue(textReportPage.contains("C"));
